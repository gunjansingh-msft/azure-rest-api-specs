...
directive:
  ...
  - where:
    ...
    suppress: GuidUsage
    reason: The uuid definition is emitted into the OpenAPI by the TypeSpec autorest emitter as an inlined form of the standard ARM common-types Encryption type (Encryption -> CustomerManagedKeyEncryption -> KeyEncryptionKeyIdentity.federatedClientId). It is a platform-standard common-types field, not a service-authored GUID, and the Context Cache API surface introduces no GUID parameter of its own. Pending ARM API review board sign-off. Same suppression pattern as Microsoft.Chaos and Microsoft.BillingTrust.
```
